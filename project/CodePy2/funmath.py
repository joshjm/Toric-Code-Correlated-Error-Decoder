...
def getnormval(distance, mean, strength, variance):
    ...
    val = (1/(math.sqrt(2*math.pi*variance)))*math.exp(-pow(distance-mean,2)/(2*variance))
    ...
    #maxval = mlab.normpdf(mean, mean, math.sqrt(variance)) #FOR NORMALIZING THE GAUSSIAN
    maxval = (1/(math.sqrt(2*math.pi*variance)))*math.exp(-pow(mean-mean,2)/(2*variance))
    #val = (1/(math.sqrt(2*math.pi*variance)))*math.exp(-pow(distance-mean,2)/(2*variance)))
    val = -val*strength/maxval+1 #invert the gaussian -> reduces distance near centre, does nothing elsewhere
    return(val*distance) #apply to the origional distance


# def getnormval2(distance, mean, strength, variance):
#     """takes an input value for a mahatten distance, and returns a reduced
#     distance if the distance is close to the correlation dist. Strength of
#     falloff is proportional to normal distribution."""
#     val = mlab.normpdf(distance, mean, math.sqrt(variance)) #looks up the normal probability density function value
#         #equivalent expresion
#     #val = (1/(math.sqrt(2*math.pi*variance)))*math.exp(-pow(distance-mean,2)/(2*variance)))
#     val = 1-val #invert the gaussian -> reduces distance near centre, does nothing elsewhere
#     return(val*distance) #apply to the origional distance


def adjustmatrix_gaussian(zM, expectedlength, codesize, strength, variance):
    ...
    return(zM)
#
# def adjustmatrix_gamma_pythag(zM):
...
#     return(zM)
#
# def adjustmatrix_gamma_manhat(zM, codesize, strength, variance):
#     #hardcoded gamma_info.txt
        ...
        return(array)
            ...
            while ErrorChains != 0: #this loop creates 1 chain
                ...
                while i != CorrLen: #here we will just create the walk.
                    ...
                    if dice==0:
                        if walk[-1][1] == 0:
                            nextstep = [walk[i][0], 1, walk[i][2]]#removed extra bracket
                        if walk[-1][1] == 1:
                            nextstep = [modpos(walk[i][0],L,-1), 0, modpos(walk[i][2],L,1)]
                    ...
                    if dice == 3:
                        if walk[-1][1] == 0:
                            nextstep = [modpos(walk[i][0],L,-1), 1, modpos(walk[i][2],L,1)]
                        if walk[-1][1] == 1:
    ...
    for i in paths:
        ...
        for j in i[1:]:
            if abs(j[0] - jprev[0])>1:#top/bottom edge jump
                if (j[0] - jprev[0])<0:#off bottom edge #TOP AND BOTTOM TREATED THE SAME
                    array[0][0][j[1]] *= -1
